(() => {
  ...
  async function buildPagesFromPdf(doc) {
    try {
      ...
      // Build pages in FLOW mode only (keeps stepping perfect)
      pages = await window.PDFExact.buildPages(
        doc,
        { container: elContent },
        {
          textMode: "flow",
          // Important: no graphics layer; keeps DOM clean and avoids "double text"
          // (If you ever need figures on specific pages, we can toggle per page later.)
          scale: 1
        }
      );
